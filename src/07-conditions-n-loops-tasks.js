...
function findFirstSingleChar(str) {
  return (
    [...str].find((a) => [...str].filter((b) => b === a).length === 1) || null
  );
}
...
function isCreditCardNumber(ccn) {
  ...
  for (; i < tmp.length; i += 2) {
    ...
    tmp = tmp
      .slice(0, i)
      .concat(digit)
      .concat(tmp.slice(i + 1));
  }
  return tmp.split('').reduce((abc, def) => +abc + +def) % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < pair.length;) {
    if (result.indexOf(pair[i]) !== -1) {
      result = result.replace(pair[i], '');
      i = 0;
    } else i += 1;
  }
  return !result;
...
function evaluateTicTacToePosition(position) {
  for (let j = 0; j < 3; j += 1) { // rows
    if (position[0][j] === position[1][j] && position[1][j] === position[2][j]
      ...
      if (position[0][j]) return position[0][j];
    }
  }
  for (let i = 0; i < 3; i += 1) { // columns
